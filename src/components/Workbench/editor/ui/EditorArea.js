...
export const renderEditorArea = ({
    ...
    return `
        <div class="flex-1 neu-inset rounded-xl ${isLight ? 'bg-white' : (isDawn ? 'bg-[#fffaf3]' : (isOceanic ? 'bg-ocean-bg' : (isNeon ? 'bg-neon-bg' : 'bg-[#0f1115]')))} overflow-hidden flex p-4 relative focus-within:ring-1 focus-within:ring-mysql-teal/50 transition-all" style="font-size:${typography.fontSize}px;line-height:${typography.lineHeight}px;font-family:${typography.fontFamily};">
            ${lineNumbersEnabled ? `<div class="flex select-none pt-1 overflow-hidden">
                <div class="w-12 ${(isLight || isDawn) ? 'text-gray-300' : (isOceanic ? 'text-ocean-text/30' : (isNeon ? 'text-neon-text/30' : 'text-gray-600'))} text-right pr-2" id="line-numbers" style="font-size:${lineNumberFontSize}px;line-height:${typography.lineHeight}px;font-family:${typography.fontFamily};"></div>
                <div class="w-5 flex flex-col items-center ${(isLight || isDawn) ? 'border-gray-100' : (isOceanic ? 'border-ocean-border/30' : (isNeon ? 'border-neon-border/30' : 'border-white/5'))} border-r" id="fold-gutter" style="line-height:${typography.lineHeight}px;"></div>
            </div>` : ''}
