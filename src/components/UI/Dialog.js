...
export class Dialog {
    static init() {
        ...
        overlay.innerHTML = `
            <div id="tactile-dialog" class="${isLight ? 'bg-white border-gray-200' : (isDawn ? 'bg-[#fffaf3] border-[#f2e9e1]' : (isOceanic ? 'bg-ocean-panel border-ocean-border/50' : (isNeon ? 'bg-neon-panel border-neon-border/50' : 'bg-[#16191e] border border-white/10')))} rounded-2xl w-[400px] shadow-2xl scale-95 transition-transform duration-200 overflow-hidden relative">
                <div class="absolute top-0 inset-x-0 h-1 bg-gradient-to-r ${isOceanic ? 'from-ocean-frost to-ocean-mint' : (isDawn ? 'from-[#ea9d34] to-[#d7827e]' : (isNeon ? 'from-neon-accent to-neon-pink' : 'from-neon-cyan to-purple-500'))}"></div>
                <div class="p-6 pt-8 text-center space-y-4">
                    ...
                    <h3 id="dialog-title" class="text-sm font-black uppercase tracking-[0.2em] ${isLight ? 'text-gray-800' : (isDawn ? 'text-[#575279]' : (isOceanic ? 'text-ocean-text' : (isNeon ? 'text-neon-text' : 'text-white')))}"></h3>
                    <p id="dialog-message" class="text-[11px] ${(isLight || isDawn) ? 'text-gray-500' : (isOceanic ? 'text-ocean-text/70' : (isNeon ? 'text-neon-text/80' : 'text-gray-400'))} font-mono leading-relaxed whitespace-pre-wrap"></p>
                </div>
    ...
    static show({ title, message, type = 'info', onConfirm = null }) {
        ...
        this.title.className = `text-sm font-black uppercase tracking-[0.2em] ${isLight ? 'text-gray-800' : (isDawn ? 'text-[#575279]' : (isOceanic ? 'text-ocean-text' : (isNeon ? 'text-neon-text' : 'text-white')))}`;
        this.message.className = `text-[11px] ${(isLight || isDawn) ? 'text-gray-500' : (isOceanic ? 'text-ocean-text/70' : (isNeon ? 'text-neon-text/80' : 'text-gray-400'))} font-mono leading-relaxed whitespace-pre-wrap`;
