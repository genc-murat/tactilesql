...
export class Dialog {
    static init() {
        ...
        overlay.innerHTML = `
            <div id="tactile-dialog" class="${isLight ? 'bg-white border-gray-200' : (isDawn ? 'bg-[#fffaf3] border-[#f2e9e1]' : (isOceanicVariant ? 'bg-ocean-panel border-ocean-border/50' : (isNeon ? 'bg-neon-panel border-neon-border/50' : 'bg-[#16191e] border border-white/10')))} rounded-2xl w-[400px] shadow-2xl scale-95 transition-transform duration-200 overflow-hidden relative">
                <div class="absolute top-0 inset-x-0 h-1 bg-gradient-to-r ${isOceanicVariant ? 'from-ocean-frost to-ocean-mint' : (isDawn ? 'from-[#ea9d34] to-[#d7827e]' : (isNeon ? 'from-neon-accent to-neon-pink' : 'from-neon-cyan to-purple-500'))}"></div>
                <div class="p-6 pt-8 text-center space-y-4">
                    <div id="dialog-icon-container" class="mx-auto size-12 rounded-full ${isLight ? 'bg-gray-50' : (isDawn ? 'bg-[#faf4ed]' : (isNeon ? 'bg-neon-accent/10 border-neon-accent/20' : 'bg-white/5'))} flex items-center justify-center mb-4">
                        <span id="dialog-icon" class="material-symbols-outlined text-2xl ${(isLight || isDawn) ? 'text-gray-400' : (isNeon ? 'text-neon-accent' : 'text-white')}">info</span>
                    </div>
                    <h3 id="dialog-title" class="text-sm font-black uppercase tracking-[0.2em] ${isLight ? 'text-gray-800' : (isDawn ? 'text-[#575279]' : (isOceanicVariant ? 'text-ocean-text' : (isNeon ? 'text-neon-text' : 'text-white')))}"></h3>
                    <p id="dialog-message" class="text-[11px] ${(isLight || isDawn) ? 'text-gray-500' : (isOceanicVariant ? 'text-ocean-text/70' : (isNeon ? 'text-neon-text/80' : 'text-gray-400'))} font-mono leading-relaxed whitespace-pre-wrap"></p>
                </div>
                <div id="dialog-actions" class="p-4 ${isLight ? 'bg-gray-50 border-gray-100' : (isDawn ? 'bg-[#faf4ed] border-[#f2e9e1]' : (isNeon ? 'bg-neon-bg/50 border-neon-border/20' : 'bg-white/5 border-white/5'))} flex gap-3 justify-center border-t">
    ...
    static show({ title, message, type = 'info', onConfirm = null }) {
        ...
        this.dialog.className = `${isLight ? 'bg-white border-gray-200' : (isDawn ? 'bg-[#fffaf3] border-[#f2e9e1]' : (isOceanicVariant ? 'bg-ocean-panel border-ocean-border/50' : (isNeon ? 'bg-neon-panel border-neon-border/50' : 'bg-[#16191e] border border-white/10')))} rounded-2xl w-[400px] shadow-2xl scale-95 transition-transform duration-200 overflow-hidden relative`;
        this.title.className = `text-sm font-black uppercase tracking-[0.2em] ${isLight ? 'text-gray-800' : (isDawn ? 'text-[#575279]' : (isOceanicVariant ? 'text-ocean-text' : (isNeon ? 'text-neon-text' : 'text-white')))}`;
        this.message.className = `text-[11px] ${(isLight || isDawn) ? 'text-gray-500' : (isOceanicVariant ? 'text-ocean-text/70' : (isNeon ? 'text-neon-text/80' : 'text-gray-400'))} font-mono leading-relaxed whitespace-pre-wrap`;
        this.actions.className = `p-4 ${isLight ? 'bg-gray-50 border-gray-100' : (isDawn ? 'bg-[#faf4ed] border-[#f2e9e1]' : (isNeon ? 'bg-neon-bg/50 border-neon-border/20' : 'bg-white/5 border-white/5'))} flex gap-3 justify-center border-t`;
    ...
    static async prompt(message, title = 'Input Required', defaultValue = '') {
        return new Promise((resolve) => {
            ...
            inputContainer.innerHTML = `
                <input type="text" id="dialog-prompt-input" class="w-full ${isLight ? 'bg-white border-gray-200 text-gray-800 focus:border-mysql-teal' : (isDawn ? 'bg-[#fffaf3] border-[#f2e9e1] text-[#575279] focus:border-[#ea9d34]' : (isOceanicVariant ? 'bg-ocean-bg border-ocean-border text-ocean-text focus:border-ocean-frost' : (isNeon ? 'bg-neon-bg border-neon-border/30 text-neon-text focus:border-neon-accent' : 'bg-[#0b0d11] border border-white/10 text-gray-300 focus:border-mysql-teal/50')))} rounded p-2 text-xs outline-none" value="${defaultValue}" />
             `;
