...
export function StoryPanel({ story, isLoading }) {
    ...
    story.sections.forEach(section => {
        ...
        if (isLight) {
            if (severity === 3) { severityClasses = 'border-red-200 bg-red-50'; iconColor = 'text-red-500'; }
            else if (severity === 2) { severityClasses = 'border-amber-200 bg-amber-50'; iconColor = 'text-amber-500'; }
            else if (severity === 1) { severityClasses = 'border-blue-200 bg-blue-50'; iconColor = 'text-blue-500'; }
            else { severityClasses = 'border-gray-100 bg-gray-50'; }
        } else if (isDawn) {
            if (severity === 3) { severityClasses = 'border-red-200 bg-[#fff5f5]'; iconColor = 'text-red-500'; }
            ...
            else if (severity === 1) { severityClasses = 'border-blue-500/30 bg-blue-500/5'; iconColor = 'text-blue-400'; }
            else { severityClasses = 'border-white/5 bg-white/5'; }
        }
        ...
        if (section.changes && section.changes.length > 0) {
            listHtml = `
                ...
                let label = change;
                let badge = '';
                if (change.startsWith('[3]')) { label = change.substring(3); badge = `<span class="px-1.5 py-0.5 rounded ${isDawn ? 'bg-red-100 text-red-600' : 'bg-red-500/20 text-red-400'} text-[8px] font-bold uppercase tracking-tighter">Critical</span>`; }
                else if (change.startsWith('[2]')) { label = change.substring(3); badge = `<span class="px-1.5 py-0.5 rounded ${isDawn ? 'bg-amber-100 text-[#ea9d34]' : 'bg-amber-500/20 text-amber-400'} text-[8px] font-bold uppercase tracking-tighter">High</span>`; }
