             ...
             <div class="w-full h-full flex flex-col px-4 py-2 overflow-y-auto custom-scrollbar">
                ...
                <div class="rounded-xl p-6 ${isLight ? 'bg-white border border-gray-200 shadow-lg' : (isNeon ? 'bg-neon-panel/20 border border-neon-border/40 shadow-[0_0_30px_rgba(0,0,0,0.3)] shadow-black/40' : (isOceanic ? 'bg-[#3B4252] border border-ocean-border/50 shadow-lg' : 'bg-[#13161b] border border-white/10 shadow-lg'))}">
                    ...
                    <h2 class="text-[11px] font-black ${isLight ? 'text-gray-900' : (isNeon ? 'text-neon-text shadow-[0_0_8px_rgba(34,211,238,0.2)]' : 'text-white')} mb-4 flex items-center gap-2 uppercase tracking-widest">
                        <span class="material-symbols-outlined ${isPostgres ? (isNeon ? 'text-neon-purple' : 'text-[#336791]') : (isNeon ? 'text-cyan-400' : 'text-mysql-teal')} text-lg">settings_input_component</span>
                    ...
                    <div class="space-y-2.5 relative z-10">
                        ...
                        <div class="grid grid-cols-4 gap-3">
                            <div class="col-span-2 space-y-1">
                                ...
                                <input name="name" class="tactile-input w-full text-xs py-1.5 ${isNeon ? '!bg-neon-bg !border-neon-border/40 !text-neon-text focus:!border-cyan-400/60' : ''}" placeholder="e.g. Production ${isPostgres ? 'PostgreSQL' : 'MySQL'}" type="text" value="${config.name || ''}" required />
                            </div>
                            <div class="col-span-2 space-y-1">
                                ...
                                <input name="database" class="tactile-input w-full text-xs py-1.5 ${isNeon ? '!bg-neon-bg !border-neon-border/40 !text-neon-text focus:!border-cyan-400/60' : ''}" placeholder="${isPostgres ? 'postgres' : 'my_database'}" type="text" value="${config.database}" />
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-4 gap-3">
                            <div class="col-span-3 space-y-1">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <input name="color-text" class="tactile-input flex-1 text-xs py-1.5 font-mono ${isNeon ? '!bg-neon-bg !border-neon-border/40 !text-neon-text' : ''}" type="text" value="${config.color || (isPostgres ? '#336791' : '#00c8ff')}" readonly />
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-4 gap-3">
                            ...
                            <div class="space-y-1">
                                ...
                                <input name="port" class="tactile-input w-full text-xs py-1.5 ${isNeon ? '!bg-neon-bg !border-neon-border/40 !text-neon-text focus:!border-cyan-400/60' : ''}" placeholder="${isPostgres ? '5432' : '3306'}" type="number" value="${config.port}" required />
                            </div>
                            <div class="space-y-1">
                                <label class="text-[9px] font-black ${isNeon ? 'text-neon-pink' : 'text-gray-500'} uppercase tracking-wider">Username</label>
                                <input name="username" class="tactile-input w-full text-xs py-1.5 ${isNeon ? '!bg-neon-bg !border-neon-border/40 !text-neon-text focus:!border-cyan-400/60' : ''}" placeholder="${isPostgres ? 'postgres' : 'root'}" type="text" value="${config.username}" required />
                            </div>
                        </div>
                        ...
                        <div class="space-y-1">
                            ...
                            <div class="relative">
                                ...
                                <button type="button" id="toggle-password" class="absolute right-2 top-1/2 -translate-y-1/2 text-gray-500 hover:${isPostgres ? (isNeon ? 'text-neon-purple' : 'text-[#336791]') : (isNeon ? 'text-cyan-400' : 'text-mysql-teal')} transition-colors" title="Show/Hide">
                        ...
                        <div class="pt-2.5 border-t ${isLight ? 'border-gray-100' : (isNeon ? 'border-neon-border/20' : 'border-white/10')}">
                            <div class="flex items-center justify-between mb-2">
                                <label class="text-[9px] font-black ${isNeon ? 'text-neon-pink' : 'text-gray-500'} uppercase tracking-wider flex items-center gap-1.5">
                                    <span class="material-symbols-outlined text-[11px] ${isNeon ? 'text-cyan-400' : ''}">security</span> SSH Tunnel
                                </label>
                                <label class="relative inline-flex items-center cursor-pointer">
                                    <input type="checkbox" name="useSSHTunnel" class="sr-only peer" ${config.useSSHTunnel ? 'checked' : ''} />
                                    <div class="w-8 h-4 ${isLight ? 'bg-gray-200' : (isNeon ? 'bg-neon-panel' : 'bg-gray-700')} rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:rounded-full after:h-3 after:w-3 after:transition-all ${isNeon ? 'peer-checked:bg-cyan-400 peer-checked:shadow-[0_0_10px_rgba(34,211,238,0.5)]' : 'peer-checked:bg-mysql-teal'}"></div>
                                </label>
                            </div>
                            ...
                            <div id="ssh-tunnel-fields" class="${config.useSSHTunnel ? '' : 'hidden'} space-y-2 transition-all">
                                <div class="grid grid-cols-4 gap-2">
                                    <div class="col-span-2 space-y-0.5">
                                        <label class="text-[8px] font-bold ${isNeon ? 'text-neon-text/60' : 'text-gray-500'} uppercase">SSH Host</label>
                                        <input name="sshHost" class="tactile-input w-full text-[11px] py-1 ${isNeon ? '!bg-neon-bg !border-neon-border/40 !text-neon-text' : ''}" placeholder="ssh.example.com" type="text" value="${config.sshHost || ''}" />
                                    </div>
                                    <div class="space-y-0.5">
                                        <label class="text-[8px] font-bold ${isNeon ? 'text-neon-text/60' : 'text-gray-500'} uppercase">SSH Port</label>
                                        <input name="sshPort" class="tactile-input w-full text-[11px] py-1 ${isNeon ? '!bg-neon-bg !border-neon-border/40 !text-neon-text' : ''}" placeholder="22" type="number" value="${config.sshPort || 22}" />
                                    </div>
                                    <div class="space-y-0.5">
                                        <label class="text-[8px] font-bold ${isNeon ? 'text-neon-text/60' : 'text-gray-500'} uppercase">SSH User</label>
                                        <input name="sshUsername" class="tactile-input w-full text-[11px] py-1 ${isNeon ? '!bg-neon-bg !border-neon-border/40 !text-neon-text' : ''}" placeholder="ubuntu" type="text" value="${config.sshUsername || ''}" />
                                    </div>
                                </div>
                                <div class="grid grid-cols-2 gap-2">
                                    <div class="space-y-0.5">
                                        <label class="text-[8px] font-bold ${isNeon ? 'text-neon-text/60' : 'text-gray-500'} uppercase">SSH Password <span class="text-gray-400">(opt)</span></label>
                                        <input name="sshPassword" class="tactile-input w-full text-[11px] py-1 ${isNeon ? '!bg-neon-bg !border-neon-border/40 !text-neon-text' : ''}" placeholder="••••••••" type="password" value="${config.sshPassword || ''}" />
                                    </div>
                                    <div class="space-y-0.5">
                                        <label class="text-[8px] font-bold ${isNeon ? 'text-neon-text/60' : 'text-gray-500'} uppercase">Key Path <span class="text-gray-400">(or password)</span></label>
                                        <input name="sshKeyPath" class="tactile-input w-full text-[11px] py-1 ${isNeon ? '!bg-neon-bg !border-neon-border/40 !text-neon-text' : ''}" placeholder="~/.ssh/id_rsa" type="text" value="${config.sshKeyPath || ''}" />
                                    </div>
                                </div>
                                <button id="test-ssh-btn" class="px-2 py-1 text-[9px] font-bold ${isLight ? 'bg-gray-100 hover:bg-gray-200 text-gray-700' : (isNeon ? 'bg-neon-pink/10 border border-neon-pink/40 text-neon-pink hover:bg-neon-pink/20 shadow-[0_0_10px_rgba(255,0,255,0.1)]' : 'bg-white/5 hover:bg-white/10 text-gray-300')} rounded flex items-center gap-1 transition-all">
                                    <span class="material-symbols-outlined text-[10px]">wifi_tethering</span>
                                    Test SSH
                                </button>
                                <span id="ssh-test-status" class="text-[9px] ${isLight ? 'text-gray-500' : (isNeon ? 'text-neon-text/40' : 'text-gray-400')}"></span>
                            </div>
                        </div>
                    </div>

                    <div class="mt-4 pt-4 border-t ${isLight ? 'border-gray-100' : (isNeon ? 'border-neon-border/20' : 'border-white/5')} flex items-center justify-between relative z-10">
                        <button id="test-btn" class="${isNeon ? 'text-neon-pink hover:text-neon-text' : 'text-gray-500 hover:' + (isLight ? 'text-gray-800' : 'text-white')} text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 transition-all">
                            <span class="material-symbols-outlined text-sm">wifi_tethering</span> Test
                        </button>
                        <button id="connect-now-btn" class="${isNeon ? 'bg-cyan-400 hover:bg-cyan-400/90 shadow-[0_0_20px_rgba(34,211,238,0.4)]' : 'gloss-btn-cyan shadow-mysql-cyan/20'} px-6 py-2.5 rounded-lg text-white text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-1.5 shadow-lg">
