...
export function SqlWorkbench() {
    ...
    container.className = `flex-1 flex flex-col h-full overflow-hidden ${isLight ? 'bg-gray-50' : (isDawn ? 'bg-[#faf4ed]' : (isOceanicVariant ? 'bg-ocean-bg' : (isNeon ? 'bg-neon-bg' : '')))}`;
    ...
    const resizerInitialBg = isLight ? 'bg-gray-200 hover:bg-mysql-teal/30' : (isDawn ? 'bg-[#f2e9e1] hover:bg-mysql-teal/30' : (isOceanicVariant ? 'bg-ocean-border/50 hover:bg-ocean-frost/30' : (isNeon ? 'bg-neon-border/50 hover:bg-neon-accent/30' : 'bg-[#0b0d11] hover:bg-mysql-teal/50')));
    ...
    sidebarResizer.innerHTML = `
        <div class="h-12 w-0.5 ${isLight || isDawn ? 'bg-gray-400' : (isOceanicVariant ? 'bg-ocean-frost/30' : (isNeon ? 'bg-neon-text/30' : 'bg-white/10'))} group-hover:bg-mysql-teal/70 rounded-full transition-colors"></div>
    `;
    ...
    const contentInitialBg = isLight ? 'bg-white' : (isDawn ? 'bg-[#fffaf3]' : (isOceanicVariant ? 'bg-ocean-bg' : (isNeon ? 'bg-neon-bg' : 'bg-[#2a2d33]')));
    ...
    const vResizerInitialBg = isLight ? 'bg-gray-100 hover:bg-mysql-teal/30' : (isDawn ? 'bg-[#f2e9e1] hover:bg-mysql-teal/30' : (isOceanicVariant ? 'bg-ocean-border/30 hover:bg-ocean-frost/30' : (isNeon ? 'bg-neon-border/30 hover:bg-neon-accent/30' : 'bg-[#1a1d23] hover:bg-mysql-teal/50')));
    ...
    verticalResizer.innerHTML = `
        <div class="w-12 h-0.5 ${isLight || isDawn ? 'bg-gray-400' : (isOceanicVariant ? 'bg-ocean-frost/30' : (isNeon ? 'bg-neon-text/30' : 'bg-white/10'))} group-hover:bg-mysql-teal/70 rounded-full transition-colors"></div>
    `;
    ...
    const onThemeChange = (e) => {
        const { theme, isLight, isDawn, isOceanicVariant: isOceanic, isNeon } = ThemeManager.getThemeFlags();

        container.className = `flex-1 flex flex-col h-full overflow-hidden transition-all duration-300 ${isLight ? 'bg-gray-50' : (isDawn ? 'bg-[#faf4ed]' : (isOceanic ? 'bg-ocean-bg' : (isNeon ? 'bg-neon-bg' : '')))}`;

        // Update resizers and areas
        const resizerBg = isLight ? 'bg-gray-200 hover:bg-mysql-teal/30' : (isDawn ? 'bg-[#f2e9e1] hover:bg-mysql-teal/30' : (isOceanic ? 'bg-ocean-border/50 hover:bg-ocean-frost/30' : (isNeon ? 'bg-neon-border/50 hover:bg-neon-accent/30' : 'bg-[#0b0d11] hover:bg-mysql-teal/50')));
        const resizerHandle = (isLight || isDawn) ? 'bg-gray-400' : (isOceanic ? 'bg-ocean-frost/30' : (isNeon ? 'bg-neon-text/30' : 'bg-white/10'));
        ...
        const contentBg = isLight ? 'bg-white' : (isDawn ? 'bg-[#fffaf3]' : (isOceanic ? 'bg-ocean-bg' : (isNeon ? 'bg-neon-bg' : 'bg-[#2a2d33]')));
        queryResults.className = `flex-1 flex flex-col overflow-hidden ${contentBg}`;

        const vResizerBg = isLight ? 'bg-gray-100 hover:bg-mysql-teal/30' : (isDawn ? 'bg-[#f2e9e1] hover:bg-mysql-teal/30' : (isOceanic ? 'bg-ocean-border/30 hover:bg-ocean-frost/30' : (isNeon ? 'bg-neon-border/30 hover:bg-neon-accent/30' : 'bg-[#1a1d23] hover:bg-mysql-teal/50')));
