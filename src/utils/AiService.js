export class AiService {
    static async generateSql(provider, apiKey, model, prompt, context) {
        switch (provider) {
            case 'openrouter':
                return await this.callOpenRouter(apiKey, model, prompt, context);
            default:
...
`;
    ...
    static async generateResponse(provider, apiKey, model, prompt, mode, context = "") {
        switch (provider) {
            case 'openrouter':
                return await this.callOpenRouter(apiKey, model, prompt, context, mode);
            default:
                throw new Error(`Unsupported AI provider: ${provider}`);
        }
    }
    ...
    static async callOpenRouter(apiKey, model, prompt, context, mode = "GEN") {
        const validModel = model && typeof model === 'string' && model.includes('/')
            ? model
            : 'openrouter/free';
        return await this.callOpenAICompatible(apiKey, 'https://openrouter.ai/api/v1/chat/completions', validModel, prompt, context, 'OpenRouter', mode);
    }
