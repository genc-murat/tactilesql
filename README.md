...
- **Native ClickHouse Integration**: High-performance connectivity via native HTTP protocol (port 8123). Features rich SVG **Visual Explain Pipeline**, **Storage Analyzer** (compression & column usage), **Query Profiler** (execution timeline & comparison), **TTL Management** (visual policy editor & impact preview), Partition Explorer, Query Log Dashboard, Kafka Monitoring, and **Merge & Mutation Monitor**.
