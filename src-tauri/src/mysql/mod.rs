...
pub(crate) fn parse_version_string(version: &str) -> (u8, u8, u8) {
...
pub(crate) fn parse_mysql_query_cost(value: &Value) -> Option<f64> {
    match value {
        Value::Object(map) => {
...
pub(crate) fn has_index_usage_signal(value: &Value, index_name: &str) -> bool {
...
pub async fn get_slow_queries(
    ...
    match rows {
        Ok(rows) => {
            ...
            for row in rows {
                let sql_text: String = row.try_get("sql_text").unwrap_or_default();
