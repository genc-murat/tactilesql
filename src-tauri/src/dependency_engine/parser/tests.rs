...
fn test_extract_dependencies_select() {
    ...
    let tables: Vec<String> = res.dependencies.iter().map(|(n, _)| n.name.clone()).collect();
    assert!(tables.contains(&"users".to_string()));
    assert!(tables.contains(&"orders".to_string()));
    assert!(res.dependencies.iter().all(|(_, t)| matches!(t, EdgeType::Select)));
}
...
fn test_extract_dependencies_insert() {
    ...
    let insert_dep = res.dependencies.iter().find(|(n, _)| n.name == "audit_log").unwrap();
    assert!(matches!(insert_dep.1, EdgeType::Insert));
    
    let select_dep = res.dependencies.iter().find(|(n, _)| n.name == "users").unwrap();
