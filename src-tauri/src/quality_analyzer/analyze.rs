...
pub async fn analyze_table_mysql(
    ...
    for (col_name, data_type) in columns {
        let null_query = format!("SELECT COUNT(*) FROM {}.{} WHERE {} IS NULL", database, table, col_name);
        let null_count: i64 = sqlx::query_scalar(&null_query)
            .fetch_one(pool)
            .await
            .unwrap_or(0);

        let distinct_query = format!("SELECT COUNT(DISTINCT {}) FROM {}.{}", col_name, database, table);
        ...
        let is_numeric = ["int", "decimal", "float", "double", "numeric"].iter().any(|t| data_type.contains(t));
        
        if is_numeric && row_count > 0 {
             let stats_query = format!("SELECT MIN({}), MAX({}), AVG({}), STDDEV({}) FROM {}.{}", col_name, col_name, col_name, col_name, database, table);
                         ...
                         if let Ok(count) = sqlx::query_scalar::<_, i64>(&outlier_query).fetch_one(pool).await {
                             if count > 0 {
                                 issues.push(DataQualityIssue {
                                     ...
                                     description: format!("Column '{}' has {} outliers (> 3 stddev).", col_name, count),
        ...
        if (col_name.contains("created_at") || col_name.contains("updated_at") || col_name.contains("timestamp")) && row_count > 0 {
             ...
             if let Ok(last_active) = sqlx::query_scalar::<_, Option<String>>(&max_date_query).fetch_one(pool).await {
        ...
        let null_pct = if row_count > 0 { (null_count as f32 / row_count as f32) * 100.0 } else { 0.0 };
        let distinct_pct = if row_count > 0 { (distinct_count as f32 / row_count as f32) * 100.0 } else { 0.0 };
    ...
    if !column_metrics.is_empty() {
        let all_cols = column_metrics.iter().map(|c| c.column_name.as_str()).collect::<Vec<_>>().join(", ");
    ...
    for (col, parent_tbl, parent_col) in fks {
        ...
        if orphan_count > 0 {
             issues.push(DataQualityIssue {
                ...
                description: format!("Found {} orphaned rows in column '{}' referencing {}.{}", orphan_count, col, parent_tbl, parent_col),
...
pub async fn analyze_table_postgres(
    ...
    // 3. Analyze Columns
    for (col_name, data_type) in columns {
        let null_query = format!("SELECT COUNT(*) FROM {}.{} WHERE {} IS NULL", schema, table, col_name);
        let null_count: i64 = sqlx::query_scalar(&null_query)
            .fetch_one(pool)
            .await
            .unwrap_or(0);

        let distinct_query = format!("SELECT COUNT(DISTINCT {}) FROM {}.{}", col_name, schema, table);
        let distinct_count: i64 = sqlx::query_scalar(&distinct_query)
            .fetch_one(pool)
            .await
            .unwrap_or(0);
        ...
        if is_numeric && row_count > 0 {
             let stats_query = format!("SELECT MIN({}), MAX({}), AVG({}::numeric), STDDEV({}::numeric) FROM {}.{}", col_name, col_name, col_name, col_name, schema, table);
