...
use crate::dependency_engine::extractor::{build_dependency_graph_mysql, build_dependency_graph_postgres};
...
pub async fn check_impact(
    ...
    let graph = match db_type {
        DatabaseType::MySQL => {
            ...
            build_dependency_graph_mysql(pool, &connection_id, None, None).await?
        },
